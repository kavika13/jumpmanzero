...
//-----------------------------------------------------------------------------
HRESULT CMusicSegment::Play( DWORD dwFlags, IDirectMusicAudioPath8* pAudioPath,long iStartTime,long iLoopStart,long iLoopEnd)
{
    ...
    m_pSegment->SetStartPoint(iStartTime);

    if(iLoopStart>=0){
        if(iLoopEnd==0){
            m_pSegment->GetLength(&iLoopEnd);
            iLoopEnd=iLoopEnd-1;
            }
        m_pSegment->SetLoopPoints(iLoopStart,iLoopEnd);
        }
