...
long SurfaceObject(long o1)
{
    ...
    while(++iLoop<MAX_OBJECTS){
        if(iRedirects[iLoop]==o1)return iLoop;
        }
...
void ScrollTexture(long iObj, float fX, float fY)
{
    ...
    if(FAILED(hr))FatalError("Error Locking triangle buffer");
...
void Clear3dData()
{
    ...
    for(iLoop=0;iLoop<MAX_TEXTURES;++iLoop){
        if(texData[iLoop])texData[iLoop]->Release();
        texData[iLoop]=NULL;
        }
    miObjects=0;
...
void LoadTexture(int iTex,char *sFile, int iType,int iAlpha)
{
   ...
   hr=D3DXCreateTextureFromFileEx(g_d3d_device, //Our D3D Device
    ...
    if(FAILED(hr))FatalError("Error loading texture");
}
...
void CopyObject(int iObject,long *iNum)
{
    ...
    while(++iLoop<MAX_OBJECTS && iPlace==-1){
        if(iRedirects[iLoop]==-1)iPlace=iLoop;
        }
...
//      vb_vertices[miVertices].color=0xFFffFFff;
...
void Reset3d(HWND hWindow)
{
    ...
    if(g_D3D==NULL)return;
    if(g_vb==NULL)return;
    if(g_d3d_device==NULL)return;
    ...
    iLoop=-1;
    while(++iLoop<MAX_TEXTURES){
        if(iLoadTexture[iLoop])LoadTexture(iLoop,RetainTextureFile[iLoop],texType[iLoop],texRequiresAlpha[iLoop]);
        }
...
long init_d3d(HWND hWindow){
   ...
   for(iLoop=0;iLoop<MAX_OBJECTS;++iLoop)iRedirects[iLoop]=-1;
...
//  hr=g_D3D->CreateDevice(D3DADAPTER_DEFAULT,D3DDEVTYPE_HAL,hWindow,D3DCREATE_SOFTWARE_VERTEXPROCESSING,&d3dpp,&g_d3d_device);
    ...
    CapsFog=1;
    if(!(dCaps.RasterCaps & D3DPRASTERCAPS_ZFOG))CapsFog=0;
...
*/
   ...
   if(FAILED(hr))FatalError("Error creating vertex buffer");
    ...
    while(++iObject<miObjects){
        if(iVis[iObject]){
            ...
            if(iLastTexture!=iTEX[iObject]){
                ...
                if(FAILED(hr))MessageBox(0,"Error setting texture!","Jumpman Zero",0);
                ...
                if(iAlphaEnabled!=iReq){
                    ...
                    if(FAILED(hr))MessageBox(0,"Error setting alpha!","Jumpman Zero",0);
                    iAlphaEnabled=iReq;
                    }
                }
