...
long CollideWall(long iX1,long iY1,long iX2,long iY2)
{
	...
	iW=-1;iLeft=0;iRight=0;iTop=0;iBottom=0;
	while(++iW<iWS){
...
void BuildNavigation()
{
	...
	while(++iLoop<iPS){
		...
		if(iPlatform>=0){
			iType=NT_Platform;
			if(iHeight<PS[iLoop].Y1-4)iType=NT_PlatformFallLeft;
		...
		if(iPlatform>=0){
			iType=NT_Platform;
			if(iHeight<PS[iLoop].Y2-4)iType=NT_PlatformFallRight;
			PS[iLoop].NavTo[PS[iLoop].Navs]=iPlatform;
			PS[iLoop].NavToType[PS[iLoop].Navs]=iType;
			++PS[iLoop].Navs;
			}
...
{
	int iLoop;
	iLoop=-1;while(++iLoop<iPS)PS[iLoop].NavDist=5000;
	iLoop=-1;while(++iLoop<iLS)LS[iLoop].NavDist=5000;

	if(iFrom<0 || iTo<0)return -1;

	if(iFromType==NT_Ladder)LS[iFrom].NavDist=0;
	if(iFromType==NT_Platform)PS[iFrom].NavDist=0;
	...
	while(++iRep<50 && !iDone){
				...
				while(++iNav<LS[iLoop].Navs){
					if(LS[iLoop].NavToType[iNav]==NT_Platform){
						...
						if(PS[iNavTo].NavDist>LS[iLoop].NavDist+1){
							...
							if(LS[iLoop].NavDist==0)PS[iNavTo].NavChoice=iNavTo;
							}
						}
					}
				}
					...
					if(iNavType!=NT_Ladder){
						...
						if(PS[iNavTo].NavDist>PS[iLoop].NavDist+1){
							...
							if(PS[iLoop].NavDist==0){
								if(iNavType==NT_Platform)PS[iNavTo].NavChoice=iNavTo;
								if(iNavType==NT_PlatformFallLeft)PS[iNavTo].NavChoice=iNavTo+2000;
								if(iNavType==NT_PlatformFallRight)PS[iNavTo].NavChoice=iNavTo+3000;
								}
							}
						}
					if(iNavType==NT_Ladder){
						...
						if(LS[iNavTo].NavDist>PS[iLoop].NavDist+1){
							...
							if(PS[iLoop].NavDist==0)LS[iNavTo].NavChoice=iNavTo+1000;
							}
						}
					}
				}
		...
		if(iToType==NT_Ladder && LS[iTo].NavDist<5000)iDone=1;
		if(iToType!=NT_Ladder && PS[iTo].NavDist<5000)iDone=1;
		}

	if(iDone==0)return -1;

	if(iToType==NT_Ladder)iChoice=LS[iTo].NavChoice;
	if(iToType!=NT_Ladder)iChoice=PS[iTo].NavChoice;
...
long ExtFunction(long iFunc, ScriptContext *SC)
{
	...
	rArg1=SC->Stack[SC->BP+0];iArg1=rArg1/256;
	rArg2=SC->Stack[SC->BP+1];iArg2=rArg2/256;
	rArg3=SC->Stack[SC->BP+2];iArg3=rArg3/256;
	rArg4=SC->Stack[SC->BP+3];iArg4=rArg4/256;
	...
	if(iFunc==EFSETSEL){
		if(iArg1==EFS_SX1)loSelected->X1=iArg2;
		else if(iArg1==EFS_SX2)loSelected->X2=iArg2;
		else if(iArg1==EFS_SY1)loSelected->Y1=iArg2;
		else if(iArg1==EFS_SY2)loSelected->Y2=iArg2;
		else if(iArg1==EFS_SZ1)loSelected->Z1=iArg2;
		else if(iArg1==EFS_SZ2)loSelected->Z2=iArg2;
		else if(iArg1==EFS_VISIBLE)loSelected->Visible=iArg2;
		else if(iArg1==EFS_NUMBER)loSelected->Num=iArg2;
		else if(iArg1==EFS_TEXTURE)loSelected->Texture=iArg2;
		else if(iArg1==EFS_EXTRA)loSelected->Extra=iArg2;
		SetObjectData(miSelectedMesh,loSelected->Texture,loSelected->Visible);
		}	
	if(iFunc==EFGETSEL){
		if(iArg1==EFS_SX1)return loSelected->X1;
		else if(iArg1==EFS_SX2)return loSelected->X2;
		else if(iArg1==EFS_SY1)return loSelected->Y1;
		else if(iArg1==EFS_SY2)return loSelected->Y2;
		else if(iArg1==EFS_SZ1)return loSelected->Z1;
		else if(iArg1==EFS_SZ2)return loSelected->Z2;
		else if(iArg1==EFS_VISIBLE)return loSelected->Visible;
		else if(iArg1==EFS_NUMBER)return loSelected->Num;
		else if(iArg1==EFS_TEXTURE)return loSelected->Texture;
		else if(iArg1==EFS_EXTRA)return loSelected->Extra;
		else if(iArg1==EFS_THIS)return loSelected->ObjectNumber;
		}	
	...
	if(iFunc==EFGET){
		if(iArg1==EFV_PX)return (long)(iPlayerX*256.0f);
		else if(iArg1==EFV_PY)return (long)(iPlayerY*256.0f);
		else if(iArg1==EFV_PZ)return (long)(iPlayerZ*256.0f);
		else if(iArg1==EFV_PSTAT)return iPlayerST*256;
		else if(iArg1==EFV_PSC)return iPlayerSC*256;
		else if(iArg1==EFV_PDIR)return iPlayerDIR*256;
		else if(iArg1==EFV_PACT)return iPlayerACT*256;
		else if(iArg1==EFV_SHOWFPS)return iShowFPS*256;
		else if(iArg1==EFV_LIVESREMAINING)return GameLivesRemaining*256;
		else if(iArg1==EFV_EVENT1)return iEvent1*256;
		else if(iArg1==EFV_EVENT2)return iEvent2*256;
		else if(iArg1==EFV_EVENT3)return iEvent3;
		else if(iArg1==EFV_EVENT4)return iEvent4;
		else if(iArg1==EFV_DEBUG)return miDEBUG*256;
		else if(iArg1==EFV_PERSPECTIVE)return miPerspective*256;
		else if(iArg1==EFV_OBJECTS)return MAX_SCRIPTOBJECTS*256;
		else if(iArg1==EFV_DONUTS)return iDS*256;
		else if(iArg1==EFV_TEXTURES)return miTextures*256;
		else if(iArg1==EFV_PLATFORMS)return iPS*256;
		else if(iArg1==EFV_LADDERS)return iLS*256;
		else if(iArg1==EFV_VINES)return iVS*256;
		else if(iArg1==EFV_WALLS)return iWS*256;	
		else if(iArg1==EFV_LEVELEXTENTX)return miLevelExtentX*256;
		else if(iArg1==EFV_THIS)return SC->ScriptReference*256;
		else if(iArg1==EFV_INPUTLEFT)return iKeyLeft*256;
		else if(iArg1==EFV_INPUTRIGHT)return iKeyRight*256;
		else if(iArg1==EFV_INPUTUP)return iKeyUp*256;
		else if(iArg1==EFV_INPUTDOWN)return iKeyDown*256;
		else if(iArg1==EFV_INPUTJUMP)return iKeyJump*256;
		else if(iArg1==EFV_INPUTATTACK)return iKeyAttack*256;
		else if(iArg1==EFV_INPUTSELECT)return iKeySelect*256;
		else if(iArg1==EFV_FREEZE)return iPlayerFreeze*256;
		else if(iArg1==EFV_SOUNDON)return GameSoundOn*256;
		else if(iArg1==EFV_MUSICON)return GameMusicOn*256;
		else if(iArg1==EFV_LASTKEY)return iLastKey*256;
		else if(iArg1==EFV_PERFORMANCE)return GamePerformance*256;
		}
	if(iFunc==EFSET){	
		if(iArg1==EFV_PX)iPlayerX=(float)rArg2/256;
		else if(iArg1==EFV_PY)iPlayerY=(float)rArg2/256;
		else if(iArg1==EFV_PZ)iPlayerZ=(float)iArg2;
		else if(iArg1==EFV_PSTAT)iPlayerST=iArg2;
		else if(iArg1==EFV_PSC)iPlayerSC=iArg2;
		else if(iArg1==EFV_PDIR)iPlayerDIR=iArg2;
		else if(iArg1==EFV_PACT)iPlayerACT=iArg2;
		else if(iArg1==EFV_EVENT1)iEvent1=iArg2;
		else if(iArg1==EFV_EVENT2)iEvent2=iArg2;
		else if(iArg1==EFV_EVENT3)iEvent3=iArg2;
		else if(iArg1==EFV_EVENT4)iEvent4=iArg2;
		else if(iArg1==EFV_PERSPECTIVE)miPerspective=iArg2;
		else if(iArg1==EFV_DEBUG)miDEBUG=iArg2;
		else if(iArg1==EFV_LEVELEXTENTX)miLevelExtentX=iArg2;
		else if(iArg1==EFV_NOROLL)iPlayerNoRoll=iArg2;
		else if(iArg1==EFV_FREEZE)iPlayerFreeze=iArg2;
		else if(iArg1==EFV_PVISIBLE)iPlayerVisible=iArg2;
		else if(iArg1==EFV_LIVESREMAINING)GameLivesRemaining=iArg2;
		return 0;
		}
	...
	if(iFunc==EFSPAWN){
		...
		iNewObject=-1;
		iLoop=-1;
		while(++iLoop<MAX_SCRIPTOBJECTS)if(!oObject[iLoop].Active && iNewObject==-1)iNewObject=iLoop;
		if(iNewObject<0){
			iNewObject=0;
			}
	...
	if(iFunc==EFATAN){
		if(rArg2==0)iVal=(rArg1>0?90:270);
		else iVal=(long)(atan((double)rArg1/(double)rArg2)*180.0f/3.1415f);
		return iVal;
		}
	if(iFunc==EFSQR){
	...
	if(iFunc==EFSERVICE){
		...
		if(iArg1==SERVICE_SETOPTION){
			if(iArg3>=0 && iArg3<=5){
				iKeyGood=0;
				iKey=iArg2;
				if(iKey==38 && iArg3==0)iKeyGood=1;
				if(iKey==40 && iArg3==1)iKeyGood=1;
				if(iKey==37 && iArg3==2)iKeyGood=1;
				if(iKey==39 && iArg3==3)iKeyGood=1;
				if(iKey==32 && iArg3==4)iKeyGood=1;
				if(iKey>='A' && iKey<='Z')iKeyGood=1;
				if(iKey>='0' && iKey<='9')iKeyGood=1;
				iLoop=-1;
				while(++iLoop<6){
					if(iArg2!=iLoop && GameKeys[iLoop]==iKey)iKeyGood=0;
					}
				if(iKeyGood)GameKeys[iArg3]=iKey;
				}
			if(iArg3==32)GameSoundOn=iArg2;
	...
	if(iFunc==EFSOUND){
		if(GameSoundOn)DoPlaySound(iArg1);
		}
	...
	if(iFunc==EFDELETE_OBJECT){
		if(iArg1==1000)SC->Active=0;
		else oObject[iArg1].Active=0;
		}
...
int FindObject(LevelObject *lObj,int iCount,int iFind)
{
	...
	iLoop=-1;
	while(++iLoop<iCount){
		if(lObj[iLoop].Num==iFind)return iLoop;
		}
	return -1;
}
...
{
	int iLoop;
	iLoop=-1;while(++iLoop<iPS)free(PS[iLoop].Mesh);
	iLoop=-1;while(++iLoop<iLS)free(LS[iLoop].Mesh);
	iLoop=-1;while(++iLoop<iDS)free(DS[iLoop].Mesh);
	iLoop=-1;while(++iLoop<iVS)free(VS[iLoop].Mesh);
	iLoop=-1;while(++iLoop<iWS)free(WS[iLoop].Mesh);
	iLoop=-1;while(++iLoop<iAS)free(AS[iLoop].Mesh);
}
...
void ComposeObject(LevelObject *lObj,long *oData,long *iPlace)
{
	...
	while(++iCopy<lObj->MeshSize){
		oData[*iPlace]=lObj->Mesh[iCopy];++*iPlace;
		}
...
void LoadLevel(char *sFileName)
{
	...
	miPerspective=0;
	miTextures=0;miMeshes=0;miScripts=0;
	iSounds=0;

	iLoop=-1;
	while(++iLoop<MAX_SCRIPTOBJECTS)oObject[iLoop].Active=0;

	iPS=0;iLS=0;iDS=0;iVS=0;iWS=0;iAS=0;
	...
	while(iPlace<iLen){
		...
		if(cData[iPlace]=='R' && cData[iPlace+1]==0){
			iPlace+=2;
			iLoop=-1;
			while(++iLoop<30)sTemp[iLoop]=cData[iPlace+iLoop];
			iPlace+=30;
			...
			iNum=-1;
			while(++iNum<iData)AS[iAS].Mesh[iNum]=StringToLong2(&cData[iPlace+(iNum<<2)]);
			iPlace+=iNum<<2;
			...
			iLoop=-1;
			while(++iLoop<8)LS[iLS].Func[iLoop]=cData[iPlace+2+iLoop];
			iPlace+=10;
			...
			iNum=-1;
			while(++iNum<iData)LS[iLS].Mesh[iNum]=StringToLong2(&cData[iPlace+(iNum<<2)]);
			iPlace+=iNum<<2;
			...
			iLoop=-1;
			while(++iLoop<8)WS[iWS].Func[iLoop]=cData[iPlace+2+iLoop];
			iPlace+=10;
			...
			iNum=-1;
			while(++iNum<iData)WS[iWS].Mesh[iNum]=StringToLong2(&cData[iPlace+(iNum<<2)]);
			iPlace+=iNum<<2;
			...
			iLoop=-1;
			while(++iLoop<8)VS[iVS].Func[iLoop]=cData[iPlace+2+iLoop];
			iPlace+=10;
			...
			iNum=-1;
			while(++iNum<iData)VS[iVS].Mesh[iNum]=StringToLong2(&cData[iPlace+(iNum<<2)]);
			iPlace+=iNum<<2;
			...
			iLoop=-1;
			while(++iLoop<8)DS[iDS].Func[iLoop]=cData[iPlace+2+iLoop];
			iPlace+=10;
			...
			iNum=-1;
			while(++iNum<iData)DS[iDS].Mesh[iNum]=StringToLong2(&cData[iPlace+(iNum<<2)]);
			iPlace+=iNum<<2;
			...
			iLoop=-1;
			while(++iLoop<8)PS[iPS].Func[iLoop]=cData[iPlace+2+iLoop];
			iPlace+=10;
			...
			iNum=-1;
			while(++iNum<iData)PS[iPS].Mesh[iNum]=StringToLong2(&cData[iPlace+(iNum<<2)]);
			iPlace+=iNum<<2;
...
long PlayerFloor()
{
	long iFloor;
	iFloor=0;
	if((iPlayerST & JS_JUMPING) && (iPlayerSC<12))iFloor=4;
    return iFloor;
}
...
long PlayerHeight()
{
	long iHeight;
	iHeight=14;
	if((iPlayerST & JS_ROLL))iHeight=7;
	if((iPlayerST & JS_PUNCH))iHeight=9;
	if((iPlayerST & JS_DYING))iHeight=7;
	return iHeight;
}
	...
	*iSupport=-1;
	...
	while(++iP<iPS){
		if(PS[iP].X1<=iX+iWide && PS[iP].X2>=iX-iWide){
			...
			iEX=iX;
			if(iEX<PS[iP].X1)iEX=PS[iP].X1;
			if(iEX>PS[iP].X2)iEX=PS[iP].X2;
			...
			bGood=0;
			if(PS[iP].Extra==3){
				if(iH<iY+1.5 && iH>=iY)bGood=1; 
				if(iPlayerST==JS_ROLL && iPlayerSC<6)bGood=0; 
				}
			else{
				if(iH<iY+iHeight)bGood=1;
				}
...
void GrabDonuts()
{
	...
	if(iGot){
		...
		else if(GameSoundOn)DoPlaySound(1);
		}
...
void GetInput()
{
	...
	iTKeyRight=iKeyRight || iTappedRight;iTappedRight=0;
	iTKeyUp=iKeyUp || iTappedUp;iTappedUp=0;
	iTKeyDown=iKeyDown || iTappedDown;iTappedDown=0;
	iTKeyJump=iKeyJump || iTappedJump;iTappedJump=0;
	iTKeyAttack=iKeyAttack || iTappedAttack;iTappedAttack=0;

	if(GetAsyncKeyState(VK_CONTROL))iTKeyJump=1;
	if(GetAsyncKeyState(VK_MENU))iTKeyAttack=1;
	...
	if(miJoystickPresent){
		...
		if(iX<15000)iTKeyLeft=1;
		if(iX>47000)iTKeyRight=1;
		if(iY<15000)iTKeyUp=1;
		if(iY>47000)iTKeyDown=1;
		if(iB&1)iTKeyJump=1;
		if(iB&2)iTKeyAttack=1;
		}

	++GameTimeInactive;
	if(iTKeyLeft+iTKeyRight+iTKeyUp+iTKeyDown+iTKeyJump)GameTimeInactive=0;
...
void AnimateDying()
{
	...
	if(iPlayerAS==0){
		...
		++iPlayerAX;
		if(iPlayerAX<5 || iPlayerAX==6)++iPlayerY;
		if(iPlayerAX>10 || iPlayerAX==8)--iPlayerY;
		if(iPlayerAX>15)iPlayerAS=1;

		++iPlayerAF;
		if(iPlayerSC<10)++iPlayerAF;
		if(iPlayerSC<5)++iPlayerAF;
		if(iPlayerSC<0){++iPlayerAF;iPlayerM=JM_DEAD;}
		iPlayerRX=iPlayerAF/-10.0f;
		if((iPlayerAF&1))iPlayerX+=iPlayerMX;
		}
	if(iPlayerAS==1){
		iPlayerM=JM_JUMPUP;
		++iPlayerAF;
		...
		if(bGrounded && iPlayerY>-5 && iSupport<iPlayerSC){
			...
			int iRand;
			iRand=rand();
			if((iRand&3)==1 && iPlayerY>30 && iPlayerX>30)iPlayerMX=-1;
			if((iRand&3)==2 && iPlayerY>30 && iPlayerX<130)iPlayerMX=1;
			iPlayerSC=(long)iSupport-3;
			iPlayerAS=0;
			iPlayerAX=0;

			if(GameSoundOn)DoPlaySound(2);
	...
	if(iPlayerAS==2){
		...
		++iPlayerAX;
		if(iPlayerAX<10 || iPlayerAX==12 || iPlayerAX==14)++iPlayerY;
		if(iPlayerAX>20 || iPlayerAX==18 || iPlayerAX==16)--iPlayerY;
		if(iPlayerAX==10 || iPlayerAX==12 || iPlayerAX==17 || iPlayerAX==20)--iPlayerZ;
		if(iPlayerAX==25)if(GameMusicOn)NewTrack2(msDeathMusic);
	...
	if(iPlayerAS==10){
		...
		if(iPlayerAF==85){
			...
			if(GameLivesRemaining==0){
				...
				ResetPlayer(0);
				if(GameMusicOn && miIntroLength!=5550)NewTrack1(msBackMusic,miIntroLength,miIntroLength);
				}
			}
		}
}
...
void ProgressGame()
{
	...
	if(!(iPlayerST & JS_DONE)){
		if(iPlayerFreeze)--iPlayerFreeze;
		if(iPlayerNoRoll)--iPlayerNoRoll;
		...
		for(iObject=0;iObject<MAX_SCRIPTOBJECTS;++iObject){
			if(oObject[iObject].Active==1)RunScript(&oObject[iObject],1);
			}
		for(iObject=0;iObject<MAX_SCRIPTOBJECTS;++iObject){
		...
		++iPlayerSC;
		if(iPlayerSC==30)if(GameMusicOn)NewTrack2(msWinMusic);
		if(iPlayerSC==300){
			LoadNextLevel();
			}
		}
...
void SetGamePerspective()
{
	...
	if(iPlayerX>-50)iPX=iPlayerX;
	iPY=iPlayerY;

	iTX=iPX/2+miLevelExtentX/4;
	iTY=iPY;
	if(iTX<35)iTX=35;
	if(iTX>miLevelExtentX-45)iTX=(float)(miLevelExtentX-45);

	iCamX=(iCamX+iTX)/2;
	iCamY=(iCamY+iTY)/2;
	if(iCamX<iTX-10 || iCamX>iTX+10)iCamX=iTX;
	if(iCamY<iTY-10 || iCamY>iTY+10)iCamY=iTY;

// PerspectiveNormal
	if(miPerspective==0)SetPerspective(iCamX,iCamY+40.0f,-115.0f,iCamX,iCamY,0.0f);
// PerspectiveCloseUp
	if(miPerspective==1)SetPerspective(iPlayerX,iCamY+35.0f,-95.0f,iPlayerX,iCamY+7,0.0f);
// Far
	if(miPerspective==2)SetPerspective(80,iCamY+50,-195.0f,80,iCamY,0);
//	SetPerspective(0,0.0f,-185.0f,0,0,0.0f);
	if(miPerspective==3)SetPerspective(iPlayerX,iCamY/2+60.0f,-110,iPlayerX,iCamY/2+32.0f,0);

// SpaceLevel
	if(miPerspective==4)SetPerspective(iPlayerX,iCamY/2+60.0f,-110,iPlayerX,iCamY/2+32.0f,0);

// Fixed (for ending)
	if(miPerspective==5)SetPerspective(70,110,-60,100,90,0);
...
{	
	...
	if(iPlayerVisible)SetObjectData(iFindMesh[iPlayerM],0,1);
...
void PrepLevel(char *sLevel)
{
	...
	if(GameMusicOn && miIntroLength!=5550)NewTrack1(msBackMusic,0,miIntroLength);
...
void LoadMenu()
{
	if(GameMenuDrawn==GameMenu)return;
	char sFileName[300];
	...
	if(GameMenu==GM_MAIN){
		...
		if(iEvent1==100){
			if(GameMusicOn)NewTrack1(msDeathMusic,0,-1);
			}
		}
                     ...
                     int       nCmdShow)
{
	...
	if(!InitMusic(hWnd))GameMusicOn=0;
	if(!InitSound(hWnd))GameSoundOn=0;
...
//	LoadNextLevel();
    ...
    { 
		...
		if( msg.message == WM_QUIT )bDone=1;
		if(bGotMsg){
		...
		if(tTime.LowPart-iTime.LowPart>iFrameTime || tTime.LowPart<iTime.LowPart){
			...
			if(((unsigned long)tTime.LowPart-iPerfTime)>(iFrameTime*40) || (unsigned long)tTime.LowPart<iPerfTime){
				...
				if(GamePerformance>99)GamePerformance=99;
				iPerfCount=0;
				}
...
//			iPerfCount=GetDrawnObjects()/10;
			...
			if(GameStatus==GS_INLEVEL){
				if(iScrollTitle>0){
					if(iEvent1==-100)iScrollTitle=1000;
					++iScrollTitle;
					iEvent1=iScrollTitle;
					RunScript(&SCTitle,1);
					if(iScrollTitle>600)iScrollTitle=0;
					}
				else{
					if(!GameFrozen)ProgressGame();
					RunScript(&SCTitle,1);
					}
				DrawGame();
				}
...
long Init3D()
{
	...
	iLoop=-1;
	while(++iLoop<100)iFindMesh[iLoop]=0;
	iLoop=-1;
	while(++iLoop<300)iOtherMesh[iLoop]=0;
	iKeyUp=0;iKeyDown=0;iKeyRight=0;iKeyLeft=0;iKeyJump=0;iKeyAttack=0;
	iTappedLeft=0;iTappedDown=0;iTappedRight=0;iTappedUp=0;iTappedJump=0;iTappedAttack=0;
	...
	if(!InitializeAll(hWnd))return 0;
	return 1;
}
...
long LoadMesh(char *sFileName)
{
	...
	iNum=-1;
	while(++iNum<iNums)oData[iNum]=StringToLong(&cData[iNum<<2]);
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, (LPCTSTR)IDI_JUMPMAN);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_ACTIVATEAPP:
		if(wParam && GameFrozen)PerformReset(hWnd);
		break;

	case WM_KEYDOWN:
		if(wParam==192)iShowFPS=1;
		if(wParam==38 || wParam==GameKeys[0])iTappedUp=iKeyUp=1;
		if(wParam==40 || wParam==GameKeys[1])iTappedDown=iKeyDown=1;
		if(wParam==37 || wParam==GameKeys[2])iTappedLeft=iKeyLeft=1;
		if(wParam==39 || wParam==GameKeys[3])iTappedRight=iKeyRight=1;
		if(wParam==32 || wParam==GameKeys[4])iTappedJump=iKeyJump=1;
		if(wParam==GameKeys[5])iTappedAttack=iKeyAttack=1;
		if(wParam==32 || wParam==13 || wParam==GameKeys[4])iKeySelect=1;

		if(wParam==81)iKeySpecial=1;
		...
		break;
	case WM_KEYUP:
		if(wParam==192)iShowFPS=0;
		if(wParam==38 || wParam==GameKeys[0])iKeyUp=0;
		if(wParam==40 || wParam==GameKeys[1])iKeyDown=0;
		if(wParam==37 || wParam==GameKeys[2])iKeyLeft=0;
		if(wParam==39 || wParam==GameKeys[3])iKeyRight=0;
		if(wParam==32 || wParam==GameKeys[4])iKeyJump=0;
		if(wParam==GameKeys[5])iKeyAttack=0;
		if(wParam==32 || wParam==13 || wParam==GameKeys[4])iKeySelect=0;

		if(wParam==81)iKeySpecial=0;
...
void DoDeathBounce()
{
	...
	int iRand;
	iRand=rand();
	if((iRand&7)==1 && iPlayerX>30)iPlayerMX=-1;
	if((iRand&7)==2 && iPlayerX<130)iPlayerMX=1;
}

int CheckWalkOff(int iCenter)
{
	if(iPlayerX<iCenter && iTKeyRight)return 0;
	if(iPlayerX>iCenter && iTKeyLeft)return 0;

	if(iTKeyDown && iPlayerY<iSitSupport-2)return 0;
...
void UpdateSituation()
{
	...
	iEvent2=-1;
	if(iSitSupport>=iPlayerY)iEvent2=iSitPlatform;
}
...
void MoveJumpmanVine()
{
	...
	if(iPlayerAF&1){
		if(iPlayerX+1>iVinX && iPlayerX-1<iVinX)iPlayerX=(float)iVinX;
		else if(iPlayerX<iVinX)++iPlayerX;
		else if(iPlayerX>iVinX)--iPlayerX;
		}
...
void MoveJumpmanLadder()
{
	...
	if(iTKeyDown && (LS[iSitLadA].Y2<iSitSupport-3 || iPlayerY>iSitSupport)){
		--iPlayerY;
		iPlayerM=(iPlayerAF&2)?JM_LADDERCLIMB2:JM_LADDERCLIMB1;
		if(LS[iSitLadA].Y2>=iSitSupport-3 && iPlayerY<iSitSupport)iPlayerY=iSitSupport;
		}
	...
	if(iPlayerX<iLadderX+1 && iPlayerX>iLadderX-1)iPlayerX=(float)iLadderX;
	else if(iPlayerX<iLadderX)iPlayerX+=1;
	else if(iPlayerX>iLadderX)iPlayerX-=1;
}
...
void MoveJumpmanNormal()
{
	...
	if(iSitSupport<iPlayerY+1 && iSitSupport>iPlayerY-1)iPlayerY=iSitSupport;
	...
	UpdateSituation();
	if(iSitSupport<iPlayerY-5 && iClimbing)iPlayerX=iPlayerOldX;
	if(PS[iSitPlatform].Extra==2){
		if(iPlayerX>iPlayerOldX)iPlayerX=iPlayerOldX;
		if(iPlayerY>iPlayerOldY)iPlayerY=iPlayerOldY;
		}
...
void MoveJumpmanFalling()
{
	...
	if(iPlayerSC>10)iPlayerY-=.5;
	if(iPlayerSC>20)iPlayerY-=.5;
...
void MoveJumpmanJumping()
{
	...
	if(iPlayerSC>50){
		if(iSitLadA!=-1){MoveJumpmanLadder();return;}
		if(iSitVinAp!=-1){MoveJumpmanVine();return;}
		MoveJumpmanNormal();return;
		}
	...
	++iPlayerSC;
	if(iPlayerSC==1)iPlayerY+=1;	
	if(iPlayerSC<5 || iPlayerSC==6 || iPlayerSC==8 || iPlayerSC==10 || iPlayerSC==12)iPlayerY+=1;	
	if(iPlayerSC>26 || iPlayerSC==25 || iPlayerSC==23 || iPlayerSC==20 || iPlayerSC==17)iPlayerY-=1;
...
void MoveJumpmanSlide()
{
	...
	if(iPlayerY<iSitSupport+1){
		if(iExtra==1){
			if(CheckJumpStart(0,0,1))return;
			++iPlayerX;
			iPlayerDIR=JD_RIGHT;
			}
		if(iExtra==2){
			if(CheckJumpStart(1,0,0))return;
			--iPlayerX;
			iPlayerDIR=JD_LEFT;
			}
		}
	...
	if(iPlayerDIR==JD_RIGHT){
		iPlayerM=JM_SLIDER;
		if(((iPlayerAF&7)==1) || ((iPlayerAF&7)==2) || ((iPlayerAF&7)==4) || ((iPlayerAF&7)==5) )iPlayerM=JM_SLIDERB;
		}
	else{
		iPlayerM=JM_SLIDEL;
		if(((iPlayerAF&7)==1) || ((iPlayerAF&7)==2) || ((iPlayerAF&7)==4) || ((iPlayerAF&7)==5) )iPlayerM=JM_SLIDELB;
		}

	if(iPlayerY<iSitSupport+2 && iPlayerY>iSitSupport-2)iPlayerY=iSitSupport;
	if(iPlayerY<iSitSupport)++iPlayerY;
	if(iPlayerY<iSitSupport)++iPlayerY;
	if(iPlayerY>iSitSupport)--iPlayerY;
	if(iPlayerY>iSitSupport)--iPlayerY;
...
void MoveJumpmanRoll()
{
	...
	if(iPlayerY<=iSitSupport){
		...
		if(!iTKeyDown){
			if(CheckJumpStart(1,1,1))return;
			}
		}
	...
	float iVel;
	iVel=1.3f;
	if(iPlayerSC>8)iVel=1;
	if(iPlayerSC>25)iVel=.7f;
	if(iPlayerSC>38)iVel=.3f;	

	if(iPlayerDIR==JD_LEFT){
		iPlayerX-=iVel;
		iPlayerM=JM_ROLLLEFT1 + ((iPlayerAF & 6)>>1);
		if(iPlayerSC<6)iPlayerM=JM_DIVELEFT;
		}
	if(iPlayerDIR==JD_RIGHT){
		iPlayerX+=iVel;
		iPlayerM=JM_ROLLRIGHT1 + ((iPlayerAF & 6)>>1);
		if(iPlayerSC<6)iPlayerM=JM_DIVERIGHT;
		}
...
void MoveJumpmanPunch()
{
	...
	if(iPlayerSC<11 && iPlayerSC!=9)++iPlayerY;
	if(iPlayerSC>12 && iPlayerSC!=14)--iPlayerY;
...
void MoveJumpman()
{
	...
	iRep=-1;while(++iRep<2){
		...
		if(iCollide==1){
			iPlayerY-=1;
			if((iPlayerST==JS_JUMPING) && iPlayerSC<15)iPlayerSC=15;
			}
		iCollide=CollideWall((long)iPlayerX-3,(long)iPlayerY+9,(long)iPlayerX+3,(long)iPlayerY+3);
		if(iCollide==3){
			++iPlayerX;
			if(iPlayerST==JS_JUMPING && iPlayerSC<15)iPlayerSC=16;
			if(iPlayerST!=JS_JUMPING && iPlayerY>iSitSupport-1 && iPlayerY>=iPlayerOldY)--iPlayerY;
			}
		if(iCollide==4){
			--iPlayerX;
			if(iPlayerST==JS_JUMPING && iPlayerSC<15)iPlayerSC=16;
