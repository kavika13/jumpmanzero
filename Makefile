...
BOOST_THREAD_LIB = libboost_thread-mt.a
...
CXXFLAGS = --std=c++14 -fvisibility=hidden -fvisibility-inlines-hidden
CPPFLAGS = -I$(ENGINE_INCLUDE_DIR) -I$(GLM_INCLUDE_DIR) -I$(LUA_INCLUDE_DIR) -I$(FMOD_INCLUDE_DIR) -I$(BOOST_INCLUDE_DIR) -I$(SYSTEM_INCLUDE_DIR)
LDFLAGS = -L$(LUA_LIB_DIR) -L$(FMOD_LIB_DIR) -L$(SYSTEM_LIB_DIR)

ENGINE_LIB_LDLIBS = -l$(LUA_LIB) -l$(FMOD_LIB) $(BOOST_LIB_DIR)/$(BOOST_SYSTEM_LIB) $(BOOST_LIB_DIR)/$(BOOST_FILESYSTEM_LIB) $(BOOST_LIB_DIR)/$(BOOST_LOG_LIB) $(BOOST_LIB_DIR)/$(BOOST_THREAD_LIB)
ENGINE_LIB_TEST_BIN_LDLIBS = $(ENGINE_LIB_LDLIBS)
GAME_BIN_LDLIBS = $(ENGINE_LIB_LDLIBS)

# Mac specific
CPPFLAGS += -F$(SDL_FRAMEWORKS_DIR) -F$(SDL_IMAGE_FRAMEWORKS_DIR)
ENGINE_LIB_TEST_BIN_LDFLAGS += -Xlinker -rpath -Xlinker @loader_path/.
GAME_BIN_LDFLAGS += -Xlinker -rpath -Xlinker @loader_path/../Frameworks
GAME_BIN_LDLIBS += -framework $(SDL_FRAMEWORK) -framework $(SDL_IMAGE_FRAMEWORK) -framework $(OPENGL_FRAMEWORK) -fobjc-link-runtime
...
# Mac specific
release: $(RELEASE_GAME_APP) $(RELEASE_ENGINE_LIB_TEST_BIN)
...
# TODO: Copy libfmod.dylib to output directory?
$(RELEASE_ENGINE_LIB_TEST_BIN): LDFLAGS += $(ENGINE_LIB_TEST_BIN_LDFLAGS)
# TODO: Should libs be included by the engine lib instead?
$(RELEASE_ENGINE_LIB_TEST_BIN): LDLIBS += $(ENGINE_LIB_TEST_BIN_LDLIBS)

$(RELEASE_GAME_BIN): $(RELEASE_GAME_BIN_OBJECTS) $(RELEASE_ENGINE_LIB)
	mkdir -p `dirname $@`
	$(LINK.cpp) $^ $(LOADLIBES) $(LDLIBS) -o $@

$(RELEASE_GAME_BIN): LDFLAGS += $(GAME_BIN_LDFLAGS)
# TODO: Should libs be included by the engine lib instead?
$(RELEASE_GAME_BIN): LDLIBS += $(GAME_BIN_LDLIBS)
...
# Mac specific
debug: $(DEBUG_GAME_APP) $(DEBUG_ENGINE_LIB_TEST_BIN)

$(DEBUG_ENGINE_LIB): $(DEBUG_ENGINE_LIB_OBJECTS)
	echo $(DEBUG_ENGINE_LIB_OBJECTS)
	mkdir -p `dirname $@`
	$(AR) $(ARFLAGS) $@ $^
...
# TODO: Copy libfmod.dylib to output directory?
$(DEBUG_ENGINE_LIB_TEST_BIN): LDFLAGS += $(ENGINE_LIB_TEST_BIN_LDFLAGS)
# TODO: Should libs be included by the engine lib instead?
$(DEBUG_ENGINE_LIB_TEST_BIN): LDLIBS += $(ENGINE_LIB_TEST_BIN_LDLIBS)

$(DEBUG_GAME_BIN): $(DEBUG_GAME_BIN_OBJECTS) $(DEBUG_ENGINE_LIB)
	mkdir -p `dirname $@`
	$(LINK.cpp) $^ $(LOADLIBES) $(LDLIBS) -o $@

$(DEBUG_GAME_BIN): LDFLAGS += $(GAME_BIN_LDFLAGS)
# TODO: Should libs be included by the engine lib instead?
$(DEBUG_GAME_BIN): LDLIBS += $(GAME_BIN_LDLIBS)
