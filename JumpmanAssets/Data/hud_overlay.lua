    ...
    for _, letter_mesh_index in ipairs(g_title_letter_mesh_indices) do
        if letter_mesh_index > 0 then
            ...
            if iX > -90 and iX < 90 then
                ...
                set_texture_and_is_visible_on_mesh(letter_mesh_index, get_loaded_texture_count() - 1, 1);  -- Title letter tex always loaded last after level loaded - TODO: Don't hard-code that?
                is_animation_still_active = true;
            else
                set_texture_and_is_visible_on_mesh(letter_mesh_index, 0, 0);
            end
        end
