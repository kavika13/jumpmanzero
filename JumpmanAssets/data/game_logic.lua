...
function Module.initialize(skip_play_level_music)
    ...
    for iChar = 0, 299 do
        ...
        if iChar >= string.byte("A") and iChar <= string.byte("Z") then
            local remapped_char_index = iChar - string.byte("A") + string.byte("a");
            character = string.char(remapped_char_index);
        elseif iChar >= string.byte("a") and iChar <= string.byte("z") then
            character = string.char(iChar);
            do_remap = true;
        elseif iChar >= string.byte("0") and iChar <= string.byte("9") then
            character = string.char(iChar);
        ...
        if is_good then
            if not do_remap then
                local new_char_mesh_index = load_mesh("data/char" .. character .. ".msh");
                g_letter_mesh_indices[iChar] = new_char_mesh_index;
            else
                g_letter_mesh_indices[iChar] = g_letter_mesh_indices[iChar - string.byte("a") + string.byte("A")];
            end
        else
