...
local g_beam_1_mesh_index = -1;
local g_beam_1_transform_indices = nil;
local g_beam_2_mesh_index = -1;
local g_beam_2_transform_indices = nil;
local g_blast_mesh_index = -1;
local g_blast_transform_index = -1;
...
local function DrawFire_(skip_next_interpolation)
    if g_is_blast_visible then
        ...
        transform_set_scale(g_blast_transform_index, 8, 8, 1);

        if g_frames_since_beam_started & 4 ~= 0 then
            transform_set_rotation_z(g_blast_transform_index, 180);
        else
            transform_clear_rotation(g_blast_transform_index);
        end

        transform_set_translation(g_blast_transform_index, iBX - 0.5, iBY, g_target_pos_z - 2);
    ...
    transform_set_translation(g_beam_1_transform_indices[1], 0.5, 0, 0);
    transform_set_scale(g_beam_1_transform_indices[2], g_gun_to_target_distance, 1, 1);
    transform_set_rotation_z(g_beam_1_transform_indices[2], g_gun_tilt_rotation);
    transform_concat_rotation_y(g_beam_1_transform_indices[2], g_gun_pan_rotation);
    transform_set_translation(g_beam_1_transform_indices[2], g_gun_pos_x, g_gun_pos_y, g_gun_pos_z);
    ...
    transform_set_rotation_x(g_beam_2_transform_indices[1], g_frames_since_beam_started * 34);
    transform_set_translation(g_beam_2_transform_indices[1], 0.5, 0, 0);
    transform_set_scale(g_beam_2_transform_indices[2], g_gun_to_target_distance, 3, 2);
    transform_set_rotation_z(g_beam_2_transform_indices[2], g_gun_tilt_rotation);
    transform_concat_rotation_y(g_beam_2_transform_indices[2], g_gun_pan_rotation);
    transform_set_translation(g_beam_2_transform_indices[2], g_gun_pos_x, g_gun_pos_y, g_gun_pos_z);
